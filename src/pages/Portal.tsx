...
/**
 * Portal page for shared bundles at /p/:id
 * Loads the bundle manifest via /api/bundles-get?id=:id and renders:
 * - Original image (from originalUrl or sourceUrl)
 * - Line Art image
 * - Palette (optional)
 * - Tips (optional)
 * - QR code (optional)
...
type ManifestShape = {
  ...
  createdAt?: string;
  originalUrl?: string;
  sourceUrl?: string; // some manifests use sourceUrl instead of originalUrl
  lineArtUrl?: string;
...
function coerceManifest(data: BundlesGetResponse, bundleId: string): Required<ManifestShape> {
  // API sometimes nests under "manifest", sometimes returns flat fields
  const base: ManifestShape = "manifest" in (data as any) && (data as any).manifest
  ...
  return {
    ...
    originalUrl: original,         // normalized to originalUrl for rendering
  ...
  if (loading) {
    return (
      <div style={{ padding: 16 }}>
        <div style={{ 
          background: "#e9d5ff", border: "1px solid #c4b5fd", color: "#3730a3",
          padding: "10px 14px", borderRadius: 8, fontWeight: 700, marginBottom: 12
        }}>
      ...
      <header>
        ...
        {m.manifestUrl && (
          <div style={{ marginTop: 4, fontSize: 13 }}>
            <a href={m.manifestUrl} target="_blank" rel="noreferrer">View manifest</a>
          </div>
        )}
      </header>
